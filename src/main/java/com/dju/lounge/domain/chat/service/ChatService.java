...
import com.dju.lounge.domain.chat.model.*;
import com.dju.lounge.domain.chat.repository.*;
...
public class ChatService {
    ...
    public void saveMessage(String roomId, ChatMessageDto chatMessageDto) {
        Chat chat = chatRepository.findById(roomId).orElseThrow(() -> new EntityNotFoundException("Chat not found"));
    ...
    public String getOrCreatePrivateChat(String otherUserId) {
        ...
        Optional<PrivateChat> chatRoom = privateChatRepository.findPrivateChatByUsers(user.getId(), otherUserId);

        if (chatRoom.isPresent()) {
            return chatRoom.get().getId();
        }

        PrivateChat newChatRoom = PrivateChat.builder()
                .user1Id(user.getId())
                .user2Id(otherUserId)
                .build();
    ...
    public List<ChatMessageResponseDto> getChatMessages(String roomId) {
        ...
        List<ChatMessage> chatMessages = chatMessageRepository.findByChatRoomIdOrderByCreatedAtAsc(roomId);
