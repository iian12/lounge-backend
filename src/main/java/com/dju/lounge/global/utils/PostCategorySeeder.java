...
public class PostCategorySeeder implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        if (categoryRepository.count() == 0) {
            List<PostCategory> categories = List.of(createCategory("free", "자유"),
                createCategory("hobby", "취미"), createCategory("work", "직장/취업/이직"),
                createCategory("school", "학업"), createCategory("finance", "재태크/경제"),
                createCategory("worries", "고민"), createCategory("love", "연애/결혼"),
                createCategory("sports", "스포츠"), createCategory("fashion", "패션"),
    ...
    private PostCategory createCategory(String slug, String name) {
        return PostCategory.builder().slug(slug).name(name).build();
    }
}
